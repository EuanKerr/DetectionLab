...
If ((Get-ScheduledTask -TaskName "AutorunsToWinEventLog" -ea silent) -eq $null)
{
    ...
    (Get-Content c:\Users\bglocaladmin\AppData\Local\Temp\windows-event-forwarding-master\AutorunsToWinEventLog\Install.ps1 -Raw) -replace 'Invoke-WebRequest -Uri "https://live.sysinternals.com/autorunsc64.exe" -OutFile "\$autorunsPath"', 'Try {
  ...
  }' | Set-Content -Path "c:\Users\bglocaladmin\AppData\Local\Temp\windows-event-forwarding-master\AutorunsToWinEventLog\Install.ps1"
    . c:\Users\bglocaladmin\AppData\Local\Temp\windows-event-forwarding-master\AutorunsToWinEventLog\Install.ps1
